...
public class PDFGenerator {
    ...
    /**
     * Fusionne plusieurs fichiers PDF en un seul fichier
     * @param pdfFiles la liste des fichiers PDF à fusionner
     * @param outputFile le fichier de sortie
     * @return le fichier PDF fusionné
     */
    public static File mergePDFs(List<File> pdfFiles, File outputFile) {
        try {
            Document document = new Document();
            PdfCopy copy = new PdfCopy(document, new FileOutputStream(outputFile));
            document.open();
            
            LOGGER.info("Fusion de " + pdfFiles.size() + " fichiers PDF");
            ...
            for (File file : pdfFiles) {
                if (file.exists() && file.isFile() && file.getName().toLowerCase().endsWith(".pdf")) {
                    try {
                        ...
                        // Ajouter toutes les pages au document
                        for (int i = 1; i <= n; i++) {
                        ...
                        LOGGER.log(Level.WARNING, "Erreur lors de la lecture du fichier PDF: " + file.getName(), e);
                    }
                }
            }
            
            document.close();
            LOGGER.info("Fusion terminée. Total de " + totalPages + " pages dans le PDF final.");
            
            return outputFile;
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "Error merging PDF files", e);
            throw new RuntimeException("Error merging PDF files", e);
        }
    }
